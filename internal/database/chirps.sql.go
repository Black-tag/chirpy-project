...
const deletChirpByID = `-- name: DeletChirpByID :exec
DELETE FROM chirps
WHERE id = $1
`

func (q *Queries) DeletChirpByID(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, deletChirpByID, id)
	return err
}
